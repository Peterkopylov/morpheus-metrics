    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE unit = %s
                ORDER BY row_order, metric_group, metric_name, metric_key
                """,
                (unit,),
            )
            return [
...
def main() -> None:
    ...
    for unit in ["b2c_moscow", "b2c_spb"]:
        ...
        dashboard = create_dashboard(
            config,
            collection_id,
            f"{unit_title(unit)} Weekly Metrics Charts (fact)",
            f"One chart per weekly metric from fact/manual_table for {unit}, comparing the last 6 weeks to the same weeks a year ago.",
        )
        ...
        for idx, metric in enumerate(metrics):
            card = create_card(
                ...
                collection_id,
                name=f"{unit_title(unit)} {metric['metric_group']} / {metric['metric_name']} (fact)",
