...
from import_erp_weekly_to_fact import (
    ...
    insert_rows,
    is_active_sale,
    is_active_seance_ticket,
)
from monthly_kpi_period_utils import month_bounds, sales_show_lookup_bounds, start_end_datetimes


REPORT_PATH = ROOT / "artifacts" / "run_reports" / "erp_monthly_to_fact_import_report.csv"
...
def is_cancelled_show(row: dict) -> bool:
    cancelled = row.get("cancelled")
    if cancelled is not None:
        return bool(cancelled)
    return int(str(row.get("status") or "1")) == 0
...
def main() -> None:
    ...
    for unit, meta in UNITS.items():
        ...
        seance_tickets = fetch_json(meta["port"], "/tickets/by-seance", {"from": fmt_ts(dt_from), "to": fmt_ts(dt_to)})
        ...
        positive_seance_tickets = [row for row in seance_tickets if is_active_seance_ticket(row)]
        revenue_show_date_general = Decimal(sum(int(str(row.get("total") or "0")) for row in positive_seance_tickets)) / Decimal(100)
        insert_payload.append(
            base_row(
                metric_ids["Revenue - Show date"],
                f"{unit}:Revenue - Show date:general:{period_start.isoformat()}",
                args.source_run_id,
                unit,
                period_start,
                period_end,
                revenue_show_date_general,
                str(revenue_show_date_general),
                {
                    "loader": "import_erp_monthly_to_fact",
                    "unit": unit,
                    "metric_name": "Revenue - Show date",
                    "scope": "general",
                    "source_endpoint": "tickets/by-seance",
                    "show_date_filter": "DeletedAt_is_null_and_positive_total",
                },
                currency_code="RUB",
            )
        )
        report_rows.append({"unit": unit, "metric_name": "Revenue - Show date", "scope": "general", "status": "inserted", "value": str(revenue_show_date_general), "reason": ""})
        ...
        seance_tickets_by_show = defaultdict(list)
        missing_show_date_join = 0
        for row in positive_seance_tickets:
            show = shows_by_show_id.get(str(row.get("seance_id")))
            if not show:
                missing_show_date_join += 1
                continue
            canonical_show = SHOW_CANON.get(show.get("event_title"))
            if not canonical_show:
                report_rows.append({"unit": unit, "metric_name": "Revenue - Show date", "scope": show.get("event_title"), "status": "skipped", "value": "", "reason": "unknown_show_name"})
                continue
            seance_tickets_by_show[canonical_show].append(row)

        if missing_show_date_join:
            report_rows.append({"unit": unit, "metric_name": "Revenue - Show date", "scope": "", "status": "skipped", "value": str(missing_show_date_join), "reason": "missing_show_date_join"})

        for show_name, rows in seance_tickets_by_show.items():
            value_revenue_show_date = Decimal(sum(int(str(row.get("total") or "0")) for row in rows)) / Decimal(100)
            insert_payload.append(
                base_row(
                    metric_ids["Revenue - Show date"],
                    f"{unit}:Revenue - Show date:show:{show_name}:{period_start.isoformat()}",
                    args.source_run_id,
                    unit,
                    period_start,
                    period_end,
                    value_revenue_show_date,
                    str(value_revenue_show_date),
                    {
                        "loader": "import_erp_monthly_to_fact",
                        "unit": unit,
                        "metric_name": "Revenue - Show date",
            ...
            report_rows.append({"unit": unit, "metric_name": "Revenue - Show date", "scope": f"show:{show_name}", "status": "inserted", "value": str(value_revenue_show_date), "reason": ""})
        ...
        for show_name, rows in shows_by_name.items():
            non_cancelled_rows = [row for row in rows if not is_cancelled_show(row)]
            cancelled_rows = [row for row in rows if is_cancelled_show(row)]
            shows_count = Decimal(len(non_cancelled_rows))
            cancelled_count = Decimal(len(cancelled_rows))
            visitors_count = Decimal(sum(int(str(row.get("guests") or "0")) for row in non_cancelled_rows))
                ...
                insert_payload.append(
                    base_row(
                        ...
                        str(value),
                        {
                            ...
                            "scope": "show",
                            "show_name": show_name,
                            "show_count_semantics": "non_cancelled_only" if metric_name == "Number of shows" else None,
                        },
                        show_name=show_name,
                    )
                )
