...
REPORT_PATH = ROOT / "artifacts" / "run_reports" / "erp_salary_variable_monthly_to_fact_import_report.csv"
...
def main() -> None:
    ...
    for unit, meta in UNITS.items():
        ...
        total_with_bonus = salary_total + bonus_total
        rows_to_insert.append(
            base_row(
                metric_id,
        ...
        report_rows.append({"unit": unit, "metric_name": "Costs - Salary variable", "scope": "general", "status": "skipped" if unallocated_bonus_total else "info", "value": str(unallocated_bonus_total), "reason": "unallocated_bonus_tail" if unallocated_bonus_total else "all_bonus_allocated"})
