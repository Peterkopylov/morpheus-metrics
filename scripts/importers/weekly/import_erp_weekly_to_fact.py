...
def is_active_seance_ticket(row: dict) -> bool:
    # `tickets/by-seance` examples do not expose `status`; use the observable ticket-level guards instead.
...
def main() -> None:
    ...
    for unit, meta in UNITS.items():
        ...
        insert_payload.append(
            base_row(
                metric_id=metric_ids["Revenue - Show date"],
                source_record_key=f"{unit}:Revenue - Show date:general:{period_start.isoformat()}",
                source_run_id=args.source_run_id,
                business_unit=unit,
                period_start=period_start,
                period_end=period_end,
                value_numeric=revenue_show_date_general,
                value_raw=str(revenue_show_date_general),
                payload={
        ...
        report_rows.append(
            {
                ...
                "scope": "general",
                "status": "inserted",
                "value": str(revenue_show_date_general),
                "reason": "",
            }
        )
                ...
                report_rows.append(
                    {
                        "unit": unit,
                        "metric_name": "Revenue - Show date",
                        "scope": show.get("event_title"),
                        "status": "skipped",
                        "value": "",
                        "reason": "unknown_show_name",
                    }
                )
                continue
            ...
            report_rows.append(
                {
                    ...
                    "scope": "",
                    "status": "skipped",
                    "value": str(missing_show_date_join),
                    "reason": "missing_show_date_join",
                }
            )
        ...
        for show_name, rows in seance_tickets_by_show.items():
            ...
            insert_payload.append(
                base_row(
                    metric_id=metric_ids["Revenue - Show date"],
                    source_record_key=f"{unit}:Revenue - Show date:show:{show_name}:{period_start.isoformat()}",
                    source_run_id=args.source_run_id,
                    business_unit=unit,
                    show_name=show_name,
                    period_start=period_start,
                    period_end=period_end,
                    value_numeric=value_revenue_show_date,
                    value_raw=str(value_revenue_show_date),
                    payload={
                        "loader": "import_erp_weekly_to_fact",
                        "unit": unit,
                        "metric_name": "Revenue - Show date",
            ...
            report_rows.append(
                {
                    ...
                    "status": "inserted",
                    "value": str(value_revenue_show_date),
                    "reason": "",
                }
            )
