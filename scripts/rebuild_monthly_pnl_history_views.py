...
Pnl_STRUCTURE_PATH = ROOT / "generated/pnl_structure_mapping_canonical.csv"
...
SELECT
    ...
    CASE
        WHEN o.source_system IN ('planfact', 'manual_dividends_total_history', 'historical_leaf_rollup_backfill', 'google_sheets_monthly_economics_historical') THEN TRUE
        ELSE FALSE
...
WITH observed_total AS (
    SELECT
        ...
        loaded_ats,
        payloads,
        'observed_planfact_total'::text AS total_origin,
        ARRAY['general', 'b2c_moscow', 'b2c_spb', 'b2b', 'franchise']::text[] AS source_business_units
...
UNION ALL
SELECT d.*
FROM derived_total d
WHERE NOT EXISTS (
    SELECT 1
    FROM observed_total o
    WHERE o.metric_id = d.metric_id
      AND COALESCE(o.show_name, '') = COALESCE(d.show_name, '')
      AND COALESCE(o.partner_name, '') = COALESCE(d.partner_name, '')
      AND COALESCE(o.channel_name, '') = COALESCE(d.channel_name, '')
      AND o.period_start = d.period_start
      AND o.period_end = d.period_end
);
