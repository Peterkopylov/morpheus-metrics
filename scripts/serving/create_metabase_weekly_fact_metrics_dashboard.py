...
FROM weekly_fact_metrics_yoy_latest_week
WHERE unit = '{unit}'
...
WHERE unit = '{unit}'
ORDER BY row_order, display_week_start, series_name;
""".strip()
...
FROM ranked
GROUP BY row_order, metric_group, metric_name, metric_key, value_type
ORDER BY row_order, metric_group, metric_name;
""".strip()
...
def main():
    ...
    dashboard = create_dashboard(
        ...
        "Fact-based weekly dashboard for Moscow and SPB: latest 6 weeks and same weeks a year ago.",
    )

    cards = [
        create_card(config, args.metabase_database_id, collection_id, "Moscow Weekly Metrics YoY Latest (fact)", latest_week_query("b2c_moscow"), "table"),
        create_card(config, args.metabase_database_id, collection_id, "SPB Weekly Metrics YoY Latest (fact)", latest_week_query("b2c_spb"), "table"),
        create_card(config, args.metabase_database_id, collection_id, "Moscow Weekly Metrics 6W Matrix (fact)", matrix_query("b2c_moscow"), "table"),
        create_card(config, args.metabase_database_id, collection_id, "SPB Weekly Metrics 6W Matrix (fact)", matrix_query("b2c_spb"), "table"),
        create_card(config, args.metabase_database_id, collection_id, "Moscow Weekly Metrics 6W Trend (fact)", trend_query("b2c_moscow"), "line"),
        create_card(config, args.metabase_database_id, collection_id, "SPB Weekly Metrics 6W Trend (fact)", trend_query("b2c_spb"), "line"),
    ]
