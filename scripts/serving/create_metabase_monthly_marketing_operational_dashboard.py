...
WITH bounds AS (
    SELECT
        COALESCE(
            [[ DATE_TRUNC('month', {{date_from}}::date)::date, ]]
            (SELECT MIN(period_start) FROM monthly_marketing_operational_latest)
        ) AS date_from,
        COALESCE(
            [[ DATE_TRUNC('month', {{date_to}}::date)::date, ]]
            (SELECT MAX(period_start) FROM monthly_marketing_operational_latest)
        ) AS date_to
),
filtered AS (
    SELECT *
    FROM monthly_marketing_operational_latest
    WHERE 1 = 1
      [[AND period_start >= DATE_TRUNC('month', {{date_from}}::date)::date]]
      [[AND period_start <= DATE_TRUNC('month', {{date_to}}::date)::date]]
)
SELECT
    DISTINCT 'Период отчета' AS "Показатель",
    TO_CHAR((SELECT date_from FROM bounds), 'MM.YYYY') || ' - ' || TO_CHAR((SELECT date_to FROM bounds), 'MM.YYYY') AS "Значение"
FROM filtered
