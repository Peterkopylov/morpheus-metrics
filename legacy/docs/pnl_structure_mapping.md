...
- в текущем leaf-only PlanFact raw-import контуре такие строки не импортируются как observed raw fact, кроме явно разрешенных верхнеуровневых значений вроде `Выручка`
- для управленческого P&L они сначала собираются системно через canonical hierarchy rollup/views
- затем нужные rollup-строки могут materialize обратно в `fact_metric_observation` как `calculated_rollup` rows с отдельным `source_system`
...
- `raw observed fact` и `materialized calculated fact` — это не одно и то же;
- но обе сущности могут жить в одном runtime table `fact_metric_observation`, если calculated rows явно помечены по происхождению;
- это позволяет читать unified monthly P&L из одного слоя без потери source trace.
