...
DEFAULT_OUTPUT_PATH = ROOT / "generated/monthly_pnl_net_profit_bridge_report.csv"
SOURCE_VIEW = "monthly_pnl_active_history_with_total"
METRICS = (
    "Revenue",
    "Variable costs",
    "Fixed costs",
    "Cost article - Налог на прибыль (доходы)",
    "Other income",
    "Net profit",
)
ZERO = Decimal("0")
...
def attention_level(abs_delta: Decimal, net_profit: Decimal) -> tuple[str, str]:
    base = max(abs(net_profit), Decimal("1"))
...
def main() -> None:
    ...
    for key in sorted(metric_values.keys(), key=lambda item: (item.business_unit, item.period_start)):
        ...
        tax = values.get("Cost article - Налог на прибыль (доходы)", ZERO)
        other_income = values.get("Other income", ZERO)
        net_profit = values.get("Net profit", ZERO)
        calculated_net_profit = revenue - variable_costs - fixed_costs - tax + other_income
        delta = net_profit - calculated_net_profit
        abs_delta = abs(delta)
        missing_metrics = [metric for metric in METRICS if metric not in values]
        review_attention, check_status = attention_level(abs_delta=abs_delta, net_profit=net_profit)
        coverage_status = "complete" if not missing_metrics else "missing_treated_as_zero"
