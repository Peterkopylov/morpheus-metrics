...
Архитектурное решение по fact-layer:
...
На текущих данных у нас подтверждены оба B2C-контура:
...
- московский CRM login + московский `city_id`
- петербургский CRM login + петербургский `city_id`
...
По состоянию на `2026-05-16` в московском кабинете Яндекс.Билетов уже появился новый спектакль:

- `Поезд, Чехов, два орла`

Для active fact-layer его нужно считать частью `b2c_show_names`.

## Практические выводы, которые уже подтверждены

### 0. Ticket sales ideology

Для вопросов про билетные продажи в fact-layer:
...
primary source должен быть **Yandex Tickets**, не ERP.

ERP для этих метрик оставляем как:

- secondary / reference source;
- operational fallback;
- временный слой до подключения СПб кабинета.
...
Это не меняет target-идеологию:

- после подключения СПб ticket-sales контур тоже должен идти через Yandex Tickets.
