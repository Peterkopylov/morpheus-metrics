...
- weekly ingestion сайта из Метрики зафиксирован в [yandex_metrica_weekly_fact_ingestion.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/yandex_metrica_weekly_fact_ingestion.md)
...
- monthly assembly logic для объединения historical sheet, PlanFact, observed total и manual dividends backfill зафиксирована в:
  - [monthly_metric_history_assembly.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/monthly_metric_history_assembly.md)
- cross-source interpretation for monthly finance metrics:
  - `PlanFact` monthly P&L remains the primary observed source for canonical monthly P&L rows such as `Revenue`, `Operating profit`, `Net profit`, `Cost article - ...`, `Costs - Salary variable`, and `Cost article - Актёры ФОТ`
  - monthly KPI contours from ERP / amoCRM / Metrica / Direct are separate source families and must not silently overwrite PlanFact monthly P&L semantics; they are used either for non-P&L business metrics or for reconciliation against P&L rows
  - for B2C salary interpretation, `PlanFact -> Cost article - Актёры ФОТ` should be compared not to bare ERP `salary_total`, but to the full ERP variable-salary contour for the same month:
    - `salary_total` from `POST /salaries/period`
    - `allocated bonuses` from `POST /bonuses` with reliable `seance_id -> show` match
    - `unallocated bonus tail` from `POST /bonuses` that cannot be safely attached to a show
  - operationally this means `PlanFact -> Cost article - Актёры ФОТ` is expected to be closest to `ERP Costs - Salary variable (general)` plus the ERP unallocated bonus tail, not to show-level ERP salary rows alone
  - ERP weekly and monthly `Costs - Salary variable (general)` must therefore carry the same semantics: `salary_total + full bonus_total`, with allocated and unallocated bonus parts preserved in payload for trace/debug
- ERP show-count semantics:
  - for ERP-derived `Number of shows`, the canonical meaning is `non-cancelled shows only`
  - ERP-derived `Number of shows cancelled` stores cancelled shows separately
  - ERP-derived `Number of show visitors` should aggregate `guests` only across non-cancelled shows, so that ratios like `Number of show visitors / Number of shows` use the same denominator semantics in weekly and monthly contours
...
Cross-source note for monthly P&L reconciliation:

- `PlanFact` monthly P&L is the canonical observed source for monthly finance rows in the active contour.
- ERP monthly salary facts are useful as a reconciliation layer for actor / variable salary, but they live in a different source family and should be interpreted through their own construction logic.
- For B2C, `Cost article - Актёры ФОТ` from PlanFact is expected to align most closely with ERP monthly variable salary after adding both:
  - bonuses allocated to matched shows;
  - bonus rows that remain unallocated because ERP does not provide a safe show match.
- Therefore, when PlanFact actor salary looks higher than ERP `Costs - Salary variable (general)` from `fact`, first check whether the ERP unallocated bonus tail is present only in payload and not in the stored numeric value.
...
- canonical count rule:
  - `Number of shows` from ERP means only non-cancelled shows
  - cancelled ERP shows must be counted into `Number of shows cancelled`, not into `Number of shows`
...
- [erp_weekly_fact_ingestion.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/erp_weekly_fact_ingestion.md)
- [erp_endpoint_metric_mapping.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/erp_endpoint_metric_mapping.md)
- [erp_salary_variable_logic.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/erp_salary_variable_logic.md)
- [erp_survey_satisfaction_logic.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/erp_survey_satisfaction_logic.md)
- [erp_site_widget_comparison.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/erp_site_widget_comparison.md)
...
Семантическое разделение для revenue:

- canonical `Revenue` в ERP sales contour означает ticket revenue по `sell date` через `POST /tickets/by-sell`;
- canonical `Revenue - Show date` является отдельной observed metric для ticket revenue по `show / seance date` через `POST /tickets/by-seance`;
- эти две метрики не взаимозаменяемы и не должны публиковаться под одним именем `Revenue`;
- по состоянию на `2026-06-05` `Revenue - Show date` входит в регулярные weekly/monthly ERP core ingestion;
- базовое active правило для `Revenue - Show date`: `DeletedAt is null` и `total > 0`, потому что в `tickets/by-seance` в подтверждённых примерах нет поля `status`, как в `tickets/by-sell`.
...
Это правило не распространяется на ERP ticket-sales semantics:

- `Revenue - Show date` не является monthly P&L child-row;
- это отдельная observed KPI metric для B2C ERP, нужная чтобы отделить month-of-sale revenue от month-of-performance revenue.
...
- [yandex_tickets_fact_layer_decision.md](/Users/Peter/Documents/Morpheus%20Metrics/legacy/docs/yandex_tickets_fact_layer_decision.md)
